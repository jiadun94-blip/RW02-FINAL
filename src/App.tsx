  ...
  if (!session) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-100 p-6 text-center">
      <form className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-sm" onSubmit={(e:any) => { e.preventDefault(); supabase.auth.signInWithPassword({ email: e.target.email.value, password: e.target.password.value }); }}>
        <h2 className="font-black mb-6 italic uppercase">Login Admin RW 02</h2>
  ...
  return (
    <div className="max-w-md mx-auto p-4 pb-24 min-h-screen bg-slate-50 font-sans">
      <div className="flex justify-between items-center mb-6">
        <h1 className="font-black italic text-xl tracking-tighter">KAS RW 02</h1>
        <button onClick={() => supabase.auth.signOut()} className="text-[10px] font-bold text-rose-500 bg-white px-3 py-1 rounded-full shadow-sm">LOGOUT</button>
      </div>
      ...
      <div className="bg-white p-5 rounded-3xl shadow-sm mb-6 border border-white">
        ...
        {desc === 'Lainnya' && <input value={customDesc} onChange={e => setCustomDesc(e.target.value)} placeholder="Keterangan manual..." className="w-full p-3 bg-yellow-50 mb-3 rounded-xl text-xs outline-none" />}
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <select value={month} onChange={e => setMonth(Number(e.target.value))} className="p-3 bg-slate-50 rounded-xl text-[10px] font-bold"> {daftarBulan.map((b, i) => <option key={b} value={i+1}>{b}</option>)} </select>
          <select value={year} onChange={e => setYear(Number(e.target.value))} className="p-3 bg-slate-50 rounded-xl text-[10px] font-bold"> <option value={2025}>2025</option><option value={2026}>2026</option> </select>
        </div>
        <div className="flex gap-2">
          <button onClick={() => handleSave('masuk')} className="flex-1 bg-emerald-600 text-white py-4 rounded-2xl font-black text-[10px]">SIMPAN MASUK</button>
          {!editingId && <button onClick={() => handleSave('keluar')} className="flex-1 bg-rose-600 text-white py-4 rounded-2xl font-black text-[10px]">SIMPAN KELUAR</button>}
        </div>
      </div>
